    ...
    <div className="w-full h-full flex flex-col items-center justify-center border border-black bg-white">
      ...
      <div className="flex-1 w-full flex items-center justify-center bg-gray-100 border-b border-black">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500">视频播放区域</p>
          <p className="text-xs text-gray-400 mt-2">{video.url || '暂无视频URL'}</p>
        </div>
      </div>
      ...
      <div className="w-full p-4 border-t border-black">
        <div className="flex justify-between items-start">
          ...
          <div className="flex gap-2">
            ...
            >
              <Heart
                className={`w-6 h-6 ${video.liked ? 'fill-black' : ''}`}
              />
            ...
            >
              <Star
                className={`w-6 h-6 ${video.favorited ? 'fill-black' : ''}`}
              />
              <span className="text-xs mt-1">{video.favoriteCount}</span>
            </button>
          </div>
        </div>
      </div>
